...
for i=1:length(MIlist)
    MIcum(i) = sum(MI(:)>MIlist(i));
end
MIcum = MIcum/(length(MI)^2)*100;
...
for i=1:length(dihtype)-1
    for j=i+1:length(dihtype)
        ...
        MItable1(count,:) = [i j ata1 ata2 atb1 atb2 MItype isallo intertorsiondis(i,j) MI(i,j)];
        count = count+1;
    end
end
...
MIavg_bb_bb_TM = mean(MItable1(ind_bb & ind_TM,10));
MIavg_bb_sc_TM = mean(MItable1(ind_bs & ind_TM,10));
MIavg_sc_sc_TM = mean(MItable1(ind_ss & ind_TM,10));
MIstd_bb_bb_TM = std(MItable1(ind_bb & ind_TM,10));
MIstd_bb_sc_TM = std(MItable1(ind_bs & ind_TM,10));
MIstd_sc_sc_TM = std(MItable1(ind_ss & ind_TM,10));

MIavg_bb_bb_loop = mean(MItable1(ind_bb & ~ind_TM,10));
MIavg_bb_sc_loop = mean(MItable1(ind_bs & ~ind_TM,10));
MIavg_sc_sc_loop = mean(MItable1(ind_ss & ~ind_TM,10));
MIstd_bb_bb_loop = std(MItable1(ind_bb & ~ind_TM,10));
MIstd_bb_sc_loop = std(MItable1(ind_bs & ~ind_TM,10));
MIstd_sc_sc_loop = std(MItable1(ind_ss & ~ind_TM,10));
