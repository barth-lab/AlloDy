function [emode, frequency, covar, covar_atom, covar_norm] = anm(crd, cutoff)
%% anm
...
%# [emode, frequency, covar, covar_atom, covar_norm] = anm(crd, cutoff);
%% Description
...
%                 [double natom3]
% * cutoff      - cutoff distance of the model. default is 15 angstroms. 
%                 [double scalar], recommended values in A.R. Atilgan et 
%                 al. are 12–15 Å.              
...
% * covar_norm  - covar_atom normalized
...
if nargout >= 5 % Output normalized covariance
  covar_norm = covar_atom;
  for i=1: size(covar_atom,1)
      for j=1:size(covar_atom,2)
          covar_norm(i,j) = covar_atom(i,j)/sqrt(covar_atom(i,i)*covar_atom(j,j));
      end
  end
end
