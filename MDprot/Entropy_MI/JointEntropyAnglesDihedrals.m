...
%
%% Usage:
% H = JointEntropyAnglesDihedrals(X, Y)
% [H, histXY] = JointEntropyAnglesDihedrals(X, Y, nbins, binrangeX)
% [H, histXY] = JointEntropyAnglesDihedrals(X, Y, nbins, binrangeX, binrangeY)
%
%% Description:
...
% column of Y
%
% * X, Y = data to be analyzed, in this case, X are the angles and Y are the
% dihedrals
%
...
function [H, histXY] = JointEntropyAnglesDihedrals(X, Y, nbins, binrangeX, binrangeY)
...
for ColumnX = 1:M1
    ...
    for ColumnY = jStart:M2
        ...
        thi = minX + xEdgew/2;
        for binX = 1:nbins(1) % angle part
            for binY = 1:nbins(2) % dihedral part
                if P(binX,binY) > 0 % Non zero prob, add it to sum
                H(ColumnX,ColumnY) = H(ColumnX,ColumnY) - P(binX,binY) .* log(P(binX,binY)/(sin(thi)*xEdgew*yEdgew));
                end
            end
            thi = thi +  xEdgew;
        end
