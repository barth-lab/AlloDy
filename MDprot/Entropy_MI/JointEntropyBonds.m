...
%
%% Usage:
% H = JointEntropyBonds(X, Y)
% [H, histXY] = JointEntropyBonds(X, Y, nbins, binrangeX)
% [H, histXY] = JointEntropyBonds(X, Y, nbins, binrangeX, binrangeY)
%
%% Description:
...
function [H, histXY] = JointEntropyBonds(X, Y, nbins, binrangeX, binrangeY)
...
%     ColumnX;
    ...
    for ColumnY = jStart:M2
        ...
        if usenbinrange % User generated range
            ...
            maxY = max(Y(:,ColumnY));
            minY = min(Y(:,ColumnY));
            assert( minY >= 0, ' Negative bonds exist, check data!!!')
            % Expand boundaries slightly
        ...
        for binX = 1:nbins(1)
            ...
            for binY = 1:nbins(2)
                ...
                H(ColumnX,ColumnY) = H(ColumnX,ColumnY) - P(binX,binY) .* log(P(binX,binY)/(bi*bi*bj*bj*xEdgew*yEdgew));
                end
                bj = bj + yEdgew;
            end
            bi = bi +  xEdgew;
