...
%
%% Usage:
% H = JointEntropyBondsAngles(X, Y)
% [H, histXY] = JointEntropyBondsAngles(X, Y, nbins, binrangeX)
% [H, histXY] = JointEntropyBondsAngles(X, Y, nbins, binrangeX, binrangeY)
%
%% Description:
...
function [H, histXY] = JointEntropyBondsAngles(X, Y, nbins, binrangeX, binrangeY)
...
for ColumnX = 1:M1
    ...
    for ColumnY = jStart:M2
        ...
        for binX = 1:nbins(1) % bond part
            thj = minY + yEdgew/2;
            for binY = 1:nbins(2) % angle part
                if P(binX,binY) > 0 % Non zero prob, add it to sum
                H(ColumnX,ColumnY) = H(ColumnX,ColumnY) - P(binX,binY) .* log(P(binX,binY)/(bi*bi*sin(thj)*xEdgew*yEdgew));
                end
                thj = thj + yEdgew;
            end
            bi = bi +  xEdgew;
